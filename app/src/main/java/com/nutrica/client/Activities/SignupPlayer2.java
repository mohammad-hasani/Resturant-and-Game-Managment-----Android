...
public class SignupPlayer2 extends AppCompatActivity {
    ...
    private void attemptLogin(Intent intent, final int city_id, final EditText edtEmail, final EditText edtPhonenumber,
...
//            showProgress(true);
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    String query = "no query" + G.spliter + "signup" + G.spliter + level + G.spliter
                            + id + G.spliter + avatar_id + G.spliter + city_id
                            + G.spliter + phonenumber + G.spliter + game_id
                            + G.spliter + creditCardNumber + G.spliter + email;
                    final String data = networkConnection.sendQuery(query);
                    networkConnection.close();
                    if (data != null)
                        if (!data.equals("false") && !data.equals("None") && data.length() > 0) {
                            startActivity(new Intent(SignupPlayer2.this, LoginActivityUser.class));
                            finish();
                        }
