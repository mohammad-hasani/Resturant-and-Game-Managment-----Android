...
public class AdminOrderFood extends Fragment {
...
//        recyclerView.addItemDecoration(new DividerItemDecoration(view.getContext(), LinearLayoutManager.VERTICAL));
        ...
        edtUsername = (EditText) view.findViewById(R.id.edt_nickname);
...
//                        String table = String.valueOf(1); //edtTableID.getText().toString();
    ...
    private void orderFood(final String username) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (data.length() < 1)
                    return;
                String query = String.format("no query%sorderfood%s%s%s%s%s", G.spliter, G.spliter, username, G.spliter, G.id, G.spliter);
                query += tmpdata;
...
//                            Toast.makeText(getContext(), getString(R.string.no)", Toast.LENGTH_LONG).show();
                        } else {
